...
> "AI agents can pay for things now — x402 cleared 169 million payments in its first year. This is the layer that decides whether your agent is *allowed* to pay. Everything you'll see is real: real signatures, real settlement, 291 end-to-end tests."
...
> "Chain-scoped signatures across fourteen chains — seven signature schemes, EVM through XRPL — live by default on Base Sepolia and Solana devnet via x402, with XRPL mainnet wired but disabled unless an operator opts in — plus Google AP2, OpenAI ACP, Stripe MPP, and Safe{Wallet}'s AllowanceModule as adapter slots behind the same pay() contract. Every wallet's history compresses into a trust score: the ledger becomes an agent credit file. Open source, Apache-2.0. `npm run verify` — 291 assertions, no mocks. The rails move the money. SpendVeto decides whether it should move."
